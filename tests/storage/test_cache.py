...
async def test_put_then_get_fresh(tmp_path: Path) -> None:
    ...
    async with await open_db(tmp_path / "jobs.db") as conn:
        await put(
            ...
            source="testerhome",
            status_code=200,
            blob_ref="raw/testerhome/20260521/aa.html.gz",
            now=t0,
        )
...
async def test_get_stale_past_24h(tmp_path: Path) -> None:
    ...
    async with await open_db(tmp_path / "jobs.db") as conn:
        await put(
            conn, url="https://x/y", source="testerhome", status_code=200, blob_ref="ref", now=t0
        )
...
async def test_non_200_is_never_fresh(tmp_path: Path) -> None:
    ...
    async with await open_db(tmp_path / "jobs.db") as conn:
        await put(
            conn, url="https://x/y", source="testerhome", status_code=429, blob_ref=None, now=t0
        )
