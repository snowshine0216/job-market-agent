...
def test_410_marks_gone() -> None:
    out = _apply_url_freshness(_job(), status_code=410, checked_at=datetime.now(UTC))
    assert out.url_status is UrlStatus.GONE


def test_500_is_transient_preserves_prior_status() -> None:
    prior = _job(url_status=UrlStatus.LIVE, url_last_checked_at=datetime(2026, 5, 1, tzinfo=UTC))
    out = _apply_url_freshness(prior, status_code=500, checked_at=datetime(2026, 5, 23, tzinfo=UTC))
    assert out.url_status is UrlStatus.LIVE
...
def test_429_is_transient_preserves_prior_status() -> None:
    prior = _job(url_status=UrlStatus.LIVE, url_last_checked_at=datetime(2026, 5, 1, tzinfo=UTC))
    out = _apply_url_freshness(prior, status_code=429, checked_at=datetime(2026, 5, 23, tzinfo=UTC))
    assert out.url_status is UrlStatus.LIVE
    assert out.url_last_checked_at == datetime(2026, 5, 1, tzinfo=UTC)


def test_3xx_is_transient_preserves_prior_status() -> None:
    prior = _job(url_status=UrlStatus.GONE, url_last_checked_at=datetime(2026, 5, 1, tzinfo=UTC))
    out = _apply_url_freshness(prior, status_code=302, checked_at=datetime(2026, 5, 23, tzinfo=UTC))
    assert out.url_status is UrlStatus.GONE
    assert out.url_last_checked_at == datetime(2026, 5, 1, tzinfo=UTC)
...
def test_other_4xx_is_transient_preserves_prior_status() -> None:
    ...
    prior = _job(url_status=UrlStatus.LIVE, url_last_checked_at=datetime(2026, 5, 1, tzinfo=UTC))
    out = _apply_url_freshness(prior, status_code=403, checked_at=datetime(2026, 5, 23, tzinfo=UTC))
    assert out.url_status is UrlStatus.LIVE
...
def test_transient_on_fresh_job_keeps_unknown_defaults() -> None:
    ...
    out = _apply_url_freshness(_job(), status_code=429, checked_at=datetime.now(UTC))
